...
import { AddMealPopover } from "@/components/calendar";
      ...
      {noHeader ? (
        ...
        <thead>
          <tr>
            <th></th>
            <th>
              <p>{weekdays[date.getDay()].substring(0, 3)}</p>
              {day}
            </th>
          </tr>
        </thead>
      ...
      <tbody>
        {mealTypes.map((mealType) => (
          <tr id={mealType} key={mealType}>
            <td
              className={`border-b p-2 border-t border-r w-1/12 sticky left-0 z-10 bg-white`}
            >
              <div className="capitalize">{mealType}</div>
            </td>
            <td className="border p-1 h-36">
              <div className="mb-2">
                {mealPlanLookup &&
                  mealPlanLookup[mealPlanKey] &&
                  mealPlanLookup[mealPlanKey].length > 0 &&
                  mealPlanLookup[mealPlanKey]
