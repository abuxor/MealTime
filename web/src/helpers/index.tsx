...
export const wait = (ms = 3000) => {
  return new Promise((res) => setTimeout(() => res("wait"), ms));
};
...
export function dataURLtoFile(
  dataurl: string,
  filename: string = Math.random().toString(36).slice(2)
) {
...
export async function imgUrlToFile(url: string) {
  ...
  if (typeof dataUrl === "string") return [dataURLtoFile(dataUrl, filename)];
...
export const getViewDateRange = (date: Date, view: string) => {
  ...
  const prevMonth =
    startOfWeek(date).getDate() > endOfWeek(date).getDate()
      ? monthNames[startOfWeek(date).getMonth() - 1]?.substring(0, 3)
      : "";
  ...
  return `${view === "day" ? day : ""} ${
    view === "week" ? `${weekStartDay} ${prevMonth} -  ${weekEndDay}` : ""
  } ${month}, ${year}`.trim();
};
...
export const truncateString = (s: string, length: number = 50) => {
